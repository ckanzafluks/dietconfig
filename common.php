...
Abstract class Common extends Module {
	...
	private function _installTables() {
		...
		$sql = 'CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_. 'diet_category_text` (
				  `id_diet_category_text` int(11) NOT NULL AUTO_INCREMENT,
				  `id_diet_category` int(11) NOT NULL,
				  `text` varchar(255) NOT NULL,
				  `url` varchar(255) NOT NULL,
				  `image` varchar(255) DEFAULT NULL,
				  `is_active` int(1) DEFAULT NULL,
				  `order` int(11) NOT NULL,
				  `datemaj` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
				  PRIMARY KEY (`id_diet_category_text`),
				  UNIQUE KEY `id_diet_category` (`id_diet_category`,`text`,`url`)
				) ENGINE=InnoDB;';
		
		$sql .= ' CREATE TABLE IF NOT EXISTS  `'._DB_PREFIX_. 'diet_category_text_log` (
		  `id_diet_category_text_log` int(11) NOT NULL AUTO_INCREMENT,
		  `sex` tinyint(1) NOT NULL,
		  `age` int(11) NOT NULL,
		  `weight` double(6,2) NOT NULL,
		  `height` double(6,2) NOT NULL,
		  `imc` double(6,2) NOT NULL,
		  `max` int(11) NOT NULL,
		  `min` int(11) NOT NULL,
		  `id_category` int(11) NOT NULL,
		  `insert_date` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
		  `id_user` varchar(255) DEFAULT NULL,
		  PRIMARY KEY (`id_diet_category_text_log`)
		) ENGINE=InnoDB;';
		
		$sql .= ' CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_. 'diet_category_result_imc_alter` (
		  `diet_category_result_imc_alter_id` int(11) NOT NULL AUTO_INCREMENT,
		  `id_category` int(10) unsigned NOT NULL,
		  `min` double(6,2) DEFAULT NULL,
		  `max` double(6,2) DEFAULT NULL,
		  `image` varchar(100) DEFAULT NULL,
		  `datemaj` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
		  PRIMARY KEY (`diet_category_result_imc_alter_id`),
	...
	private function _uninstallTables() {
		$sql1 = 'DROP TABLE  `'._DB_PREFIX_. 'diet_category_text`;';
		$sql2 = 'DROP TABLE `'._DB_PREFIX_. 'diet_category_text_log`;';
		$sql3 = 'DROP TABLE `'._DB_PREFIX_. 'diet_category_result_imc_alter`;';
	...
	{
		$this->context->controller->addJS($this->_path .'js/dietconfiguration.js');
		$this->context->controller->addCSS($this->_path .'css/dietconfiguration.css');	
	}
	##################################### END HEADERS INIT #######################################################
